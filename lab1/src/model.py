...
class ParserModel(nn.Module):
    ...
    def create_embeddings(self, word_embeddings):
        ...
        """
        # *** BEGIN YOUR CODE ***
        self.word_embeddings = torch.tensor(word_embeddings,requires_grad=True)      # create tensor of word_embeddings
        # Initialize tag and deprel embeddings using he_initializers, with requires_grad built in
    ...
    def create_weights_biases(self):
        ...
        """
        # *** BEGIN YOUR CODE ***
        w_h_size = (self.config.n_word_features+self.config.n_tag_features+self.config.n_deprel_features)*self.config.embed_size
        self.W_h = he_initializer((w_h_size, self.config.hidden_size))       # Initalize weights for hidden and output
        self.W_o = he_initializer((self.config.hidden_size,self.config.n_classes)) # layers
        self.b_h = torch.zeros([self.config.hidden_size,],requires_grad=True)   # Create hidden and output layer biases
        self.b_o = torch.zeros([self.config.n_classes,],requires_grad=True)     # with requires_grad turned on
        return
        # *** END YOUR CODE ***
    ...
    def embedding_lookup(self, id_batch, n_ids, embedding_matrix):
        ...
        """
        # *** BEGIN YOUR CODE ***
        one_hot_id_batch = one_hot_float(id_batch, n_ids)       # Perform onehot encoding
        embedded_batch = torch.mm(one_hot_id_batch.view(-1, n_ids), embedding_matrix)   # matrix multiply 3Dx2D matrix
        embedded_batch = embedded_batch.view(-1, id_batch.size()[1], self.config.embed_size) # flatten multiplied matrix
        b,n, embed_size = embedded_batch.size()
        embedded_batch=embedded_batch.reshape([b, n*embed_size],-1)     # Reshape embedded batch to desired size
        # *** END YOUR CODE ***
    ...
    def forward(self, word_id_batch, tag_id_batch, deprel_id_batch):
        ...
        # *** BEGIN YOUR CODE ***
        h = F.relu(torch.mm(x, self.W_h)+ self.b_h)  # Perform linear step through hidden layer                                                 # Apply hidden layer activation
        h_drop = F.dropout(h, self.config.dropout, training=self.training)   # Perform dropout if training
    ...
    def get_loss(self, prediction_batch, class_batch):
        ...
        """
        # *** BEGIN YOUR CODE ***
        loss = F.cross_entropy(prediction_batch,class_batch)        # Initialize loss function
        # *** END YOUR CODE ***
        return loss
    ...
    def add_optimizer(self):
        ...
        """
        # *** BEGIN YOUR CODE ***
        params = [self.W_h, self.b_h, self.W_o, self.b_o, self.word_embeddings, self.tag_embeddings, self.deprel_embeddings]
        self.optimizer = torch.optim.Adam(params=params ,lr=self.config.lr) # Initialize optimizer with parameters
        # *** END YOUR CODE ***
