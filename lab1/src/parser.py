...
class PartialParse(object):
    ...
    def parse_step(self, transition_id, deprel=None):
        ...
        if transition_id == self.left_arc_id:
            if len(self.stack) < 3:       # Can't perform a left arc if there is only two elements in the stack
                raise ValueError('Cannot perform left arc on stack of length', len(self.stack))
            self.arcs.append((self.stack[-1], self.stack[-2], deprel))      # Add arc to arcs list
            self.stack.pop(-2)                                              # Remove second last element of stack
        elif transition_id == self.right_arc_id:
            if len(self.stack) < 2:      # Can't perform right arc with only one element in stack
                raise ValueError('Cannot perform right arc on stack of length',len(self.stack))
            self.arcs.append((self.stack[-2], self.stack[-1], deprel))      # Add arc to arcs list
            self.stack.pop()                                                # Remove last element of stack
        elif transition_id == self.shift_id:
            if self.next == len(self.sentence):     # Can't shift if buffer is empty
                raise ValueError('Cannot perform shift on empty buffer')
            self.stack.append(self.next)            # Add next word to stack
            self.next += 1                          # Increment the next word index
        else:
            # Only accepts rightarc, leftarc and shift transitions
            raise ValueError('Cannot perform transition', transition_id,' please enter a valid ID')
        # *** END YOUR CODE ***
    ...
    def get_n_leftmost_deps(self, sentence_idx, n=None):
        ...
        # *** BEGIN YOUR CODE ***
        deps = []
        for arc in self.arcs:           # Check for index as head idx of arcs list
            if arc[0] == sentence_idx:
                deps += [arc[1]]        # If left arc found, add to dependency list
        if n == None or n > len(deps):  # If found less than n or want all dependencies, return entire list
            return deps
        else:
            return deps[:n]             # Otherwise, return the first n leftmost dependencies
        # *** END YOUR CODE ***
        return deps
    ...
    def get_n_rightmost_deps(self, sentence_idx, n=None):
        ...
        # *** BEGIN YOUR CODE ***
        deps = []
        for arc in self.arcs[::-1]:             # iterate through arcs backwards to find rightmost first
            if arc[0] == sentence_idx:          # if index in arcs as head idx add to dependency list
                deps += [arc[1]]
        if n == None or n > len(deps):          # If found less than n or want all dependencies, return entire list
            return deps
        else:
            return deps[:n]                     # Otherwise return the first n rightmost dependencies
        # *** END YOUR CODE ***
        return deps
    ...
    def get_oracle(self, graph):
        ...
        *ALSO IMPORTANT* make sure to use the values `self.left_arc_id`,
        ...
        # *** BEGIN YOUR CODE ***
        self.sentence = get_sentence_from_graph(graph, True)        # Set sentence
        if len(self.stack)<3:
            if self.next != len(self.sentence):                     # if there are only two elements in stack and buffer
                return self.shift_id, deprel                        # is not empty you can not arc, must shift
            if self.next == len(self.sentence):                     # if there are only two elements in stack and buffer
                    for dep_label, indicies in graph.nodes[self.stack[-2]]['deps'].items():
                        if self.stack[-1] in indicies:
                            deprel = dep_label
                    # print(self.root_tag,"AAA")
                            return self.right_arc_id, deprel             # is empty, rightarc the root onto the main verb
        last_element = self.stack[-1]
        second_last = self.stack[-2]
        """ IF LEFT ARC <- """
        for dep_label, indicies in graph.nodes[last_element]['deps'].items():   # check if second last element is in
            if second_last in indicies:                                         # dependency list of last element
                deprel = dep_label                                              # find dep label
                return self.left_arc_id, deprel                                 # if so, perform left arc
        """ IF RIGHT ARC -> """
        if graph.nodes[last_element]['head'] == second_last:                    # Check if second last element is head
            r_deps = get_right_deps(graph.nodes[last_element])                  # of last element
            r_deps = [dep for dep in r_deps]
            x = [index for index in r_deps if index >= self.next]               # check to make sure all right arcs/deps
            if len(x) == 0:                                                     # of last element have been completed
                for dep_label, indices in graph.nodes[second_last]['deps'].items():
                    if last_element in indices:                                 # Find dep label
                        deprel = dep_label                                      # if all right arcs/deps of last element
                        return self.right_arc_id, deprel                        # completed, perform right arc
        if (self.next != len(self.sentence)):                                   # otherwise if buffer not empty, shift
            return self.shift_id, 5
            # NOTE* -1 transition_id returned when graph is not projective
        # *** END YOUR CODE ***
...
def minibatch_parse(sentences, model, batch_size):
    ...
    """
    # *** BEGIN YOUR CODE ***
    partial_parses = [PartialParse(sentence) for sentence in sentences]     # create PartialParse object for each sample
    unfinished_parses = partial_parses[:]                                   # create a copy of partial_parses to iterate
    while len(unfinished_parses) != 0:
        minibatch = unfinished_parses[:batch_size]          # select minibatch of size batch_size from all samples
        parsed_minibatch = model.predict(minibatch)         # predict minibatch
        for i, parse_item in enumerate(parsed_minibatch):   # for each prediction try to perform the transformation
            try:
                minibatch[i].parse_step(parse_item[0], parse_item[1])
                if  minibatch[i].complete:                  # if transformation completes graph, remove from list
                    unfinished_parses.remove(minibatch[i])
            except ValueError:
                unfinished_parses.remove(minibatch[i])      # ignore any graph predictions that fail or are projective
    return [sent.arcs for sent in partial_parses]           # return the 'labelled' arcs for each sentence/sample
    # *** END YOUR CODE ***
    return arcs
