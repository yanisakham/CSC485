...
class PartialParse(object):
    ...
    def get_n_leftmost_deps(self, sentence_idx, n=None):
        ...
        """
        if len(self.arcs) == 0:
            raise ValueError("No Arcs Found")
        deps = []
        for arc in self.arcs:
            if arc[0] == sentence_idx:
                deps += [arc[1]]
        if n == None or n > len(deps):
            return deps
        else:
            return deps[:n]
        ...
        *ALSO IMPORTANT* make sure to use the values `self.left_arc_id`,
        ...
        Args:
            graph : nltk.parse.dependencygraph.DependencyGraph
                A projective dependency graph to head towards                with the correct dependency relation label
        ...
        last_element = self.stack[-1]                       # Last element of the stack

        l_deps = get_left_deps(graph.nodes[last_element])
        l_deps = [dep for dep in l_deps]               # Get all left dependencies of the last element
        dir_left_dep = None
        try:
            dir_left_dep = max(list(set(l_deps) & set(self.stack))) # Find the leftmost dependency within stack
            print("dld",dir_left_dep)
        except ValueError:                                      # Catch case where none exists
            pass

        r_deps = get_right_deps(graph.nodes[last_element])
        r_deps = [r for r in r_deps]                    # Get all right dependencies of last element
        dir_right_dep = None
        try:
            dir_right_dep = min(dep for dep in r_deps if dep >=self.next) # Find rightmost dependency in buffer
        except ValueError:
            pass                                # Catch if none exist

        head = graph.nodes[last_element]['head']         # Find head of element
        if head == False:
            head = 0                                # Handle case when element is ROOT

        if dir_left_dep and len(self.stack) > 2:  # Prioritize any possible left-arcs
            for dep_label, indices in graph.nodes[last_element]['deps'].items():
                if dir_left_dep in indices:         # Find Dep Label for transition
                    deprel = dep_label
                    transition_id = self.left_arc_id
                    break

        elif not dir_right_dep and head in self.stack and len(self.stack) > 1:  # Next prioritize possible right arcs
            for dep_label, indices in graph.nodes[head]['deps'].items():
                if last_element in indices:
                    deprel = dep_label
                    transition_id = self.right_arc_id
                    break
        elif self.next != len(self.sentence):   # Otherwise, shift
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) != 0:
        ...
        parsed_minibatch = model.predict(minibatch)
        for i, parse_item in enumerate(parsed_minibatch):
            try:
                ...
                unfinished_parses.remove(minibatch[i])
    return [sent.arcs for sent in partial_parses]
